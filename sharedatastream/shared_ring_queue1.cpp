...
#ifdef _WIN32
		std::memcpy(header_->metadata, metadata.data(), min(metadata.size(), (size_t)METADATA_SIZE));
#else
		std::memcpy(header_->metadata, metadata.data(), std::min(metadata.size(), (size_t)METADATA_SIZE));
#endif
