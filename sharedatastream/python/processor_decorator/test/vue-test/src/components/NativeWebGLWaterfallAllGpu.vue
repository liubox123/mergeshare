...
function uploadAndResample(newData) {
  ...
  // 补齐数据到 rowLen * rowCount
  let padded;
  if (newData.length < inputTexRowLen * inputTexRowCount) {
    padded = new Float32Array(inputTexRowLen * inputTexRowCount);
    padded.set(newData, 0);
    // 其余为0或Y_MIN
  } else {
    padded = newData;
  }
...
function onMouseMove(e) {
  if (isDragging) {
    ...
    // 悬停 tooltip
    if (!canvas.value || !historyTex) return;
    const rect = canvas.value.getBoundingClientRect();
    ...
    if (N > 1 && maxHistory.value > 0) {
      ...
      const val = historyTex[frameIdx * N + idx] ?? Y_MIN;
