...
const props = defineProps({
  waterfallData: { type: [Array, Float32Array], default: () => [] },
  wsUrl: { type: String }, // 兼容老接口但不再使用
  freqMin: { type: Number, default: 0 },
  freqMax: { type: Number, default: 80e9 },
  height: { type: Number, default: 400 },
  width: { type: Number, default: 800 },
  maxHistory: { type: Number, default: 50 }, // 历史帧数
});
const {
  waterfallData, freqMin, freqMax, maxHistory,
} = toRefs(props);
...
let gl = null;
let program = null;
let resampleProgram = null; // 下采样程序
let inputTex = null; // 输入纹理
let historyTex = null; // 历史纹理
let resultTex = null; // 新增：下采样结果临时纹理
let resampleFBO = null; // 用于下采样的FBO
let vao = null;
let uLocs = {};
let resampleULocs = {}; // 下采样着色器的uniform位置
let inputBuffer = null; // 持久的输入缓冲区
...
let inputTexRowLen = 0;
let inputTexRowCount = 0;
let maxTexSize = 0;
...
uniform int u_dstWidth;
uniform int u_rowLen;
uniform int u_rowCount;

void main() {
  float x = v_uv.x;
  int dstIdx = int(x * float(u_dstWidth));
  // 计算原始区间
  float srcStep = float(u_srcWidth) / float(u_dstWidth);
  int left = int(float(dstIdx) * srcStep);
  int right = int(float(dstIdx + 1) * srcStep);
  right = min(right, u_srcWidth);

  float maxVal = -1e20;
  for (int i = 0; i < 65536; ++i) { // 65536为极大常量，近似无限池化窗口
    int idx = left + i;
    if (idx >= right) break;
    int row = idx / u_rowLen;
    int col = idx % u_rowLen;
    float val = texelFetch(u_inputTex, ivec2(col, row), 0).r;
...
void main() {
  ...
  tx = clamp(tx, 0.0, 1.0);
  // 历史帧映射到纹理y
  float yIdx = float(u_yOffset + int(float(u_H) * y) + u_historyWriteIdx);
  float ty = mod(yIdx, float(u_H)) / float(u_H);
  ty = clamp(ty, 0.0, 1.0);
  // 查 float32 纹理
...
function initWebGL() {
  ...
  // 创建着色器程序
  program = createProgram(gl, VERT, RENDER_FRAG);
  resampleProgram = createProgram(gl, VERT, RESAMPLE_FRAG);
  ...
  // 创建FBO用于下采样，绑定resultTex
  resampleFBO = gl.createFramebuffer();
  ...
  uLocs = {
    ...
    u_historyWriteIdx: gl.getUniformLocation(program, 'u_historyWriteIdx'),
  };

  resampleULocs = {
    u_inputTex: gl.getUniformLocation(resampleProgram, 'u_inputTex'),
    u_srcWidth: gl.getUniformLocation(resampleProgram, 'u_srcWidth'),
    u_dstWidth: gl.getUniformLocation(resampleProgram, 'u_dstWidth'),
    u_rowLen: gl.getUniformLocation(resampleProgram, 'u_rowLen'),
    u_rowCount: gl.getUniformLocation(resampleProgram, 'u_rowCount'),
  };
}

// 上传新数据到输入纹理并进行GPU下采样
function uploadAndResample(newData) {
  if (!gl || !inputTex || !historyTex || N === 0) return;

  // 计算分块参数
  inputTexRowLen = Math.min(N, maxTexSize);
  inputTexRowCount = Math.ceil(N / inputTexRowLen);
  const targetN = Math.min(N, MAX_POINTS_PER_FRAME);
  const requiredSize = inputTexRowLen * inputTexRowCount;

  // 只在必要时才重新分配缓冲区
  if (!inputBuffer || inputBuffer.length < requiredSize) {
    inputBuffer = new Float32Array(requiredSize);
    inputBuffer.fill(Y_MIN);
  }

  // 如果输入数据不够大，使用预分配的缓冲区
  const uploadData = newData.length >= requiredSize ? newData : inputBuffer;
  if (newData.length < requiredSize) {
    // 只更新有效数据部分
    inputBuffer.fill(Y_MIN); // 重置缓冲区
    inputBuffer.set(newData); // 设置新数据
  }
  ...
  // FBO绑定resultTex
  gl.bindFramebuffer(gl.FRAMEBUFFER, resampleFBO);
  ...
  // 执行下采样到resultTex
  gl.viewport(0, 0, targetN, 1);
  gl.useProgram(resampleProgram);
  gl.activeTexture(gl.TEXTURE0);
  gl.bindTexture(gl.TEXTURE_2D, inputTex);
  gl.uniform1i(resampleULocs.u_inputTex, 0);
  gl.uniform1i(resampleULocs.u_srcWidth, N);
  gl.uniform1i(resampleULocs.u_dstWidth, targetN);
  gl.uniform1i(resampleULocs.u_rowLen, inputTexRowLen);
  gl.uniform1i(resampleULocs.u_rowCount, inputTexRowCount);
  gl.bindVertexArray(vao);
  gl.drawArrays(gl.TRIANGLE_STRIP, 0, 4);
  gl.bindVertexArray(null);

  // 拷贝resultTex的第0行到historyTex的historyWriteIdx行
  gl.bindFramebuffer(gl.READ_FRAMEBUFFER, resampleFBO);
  gl.bindTexture(gl.TEXTURE_2D, historyTex);
  gl.copyTexSubImage2D(
    gl.TEXTURE_2D,
    0,
    0,
    historyWriteIdx, // xoffset, yoffset
    0,
    0, // x, y in FBO
    targetN,
    1, // width, height
  ...
  // 更新缓存
  if (lastFrameData && lastFrameData.length === targetN) {
    gl.bindFramebuffer(gl.FRAMEBUFFER, resampleFBO);
    gl.readPixels(0, 0, targetN, 1, gl.RED, gl.FLOAT, lastFrameData);
    gl.bindFramebuffer(gl.FRAMEBUFFER, null);
  }

  historyWriteIdx = (historyWriteIdx + 1) % maxHistory.value;
}

function renderWaterfall() {
  if (!gl || !program || !historyTex) return;
  ...
  gl.uniform1i(uLocs.u_historyWriteIdx, historyWriteIdx);
  gl.bindVertexArray(vao);
  gl.drawArrays(gl.TRIANGLE_STRIP, 0, 4);
  gl.bindVertexArray(null);
}
...
function onMouseMove(e) {
  if (isDragging) {
    ...
    // 悬停 tooltip
    if (!canvas.value || !historyTex || !lastFrameData) return;
    const rect = canvas.value.getBoundingClientRect();
    ...
    if (N > 1 && maxHistory.value > 0) {
      ...
      const idx = Math.floor(
        ((freq - freqMin.value) / (freqMax.value - freqMin.value)) * (lastFrameData.length - 1),
      );

      // 使用缓存的数据
      const val = frameIdx === 0 && idx >= 0 && idx < lastFrameData.length
        ? lastFrameData[idx]
        : Y_MIN;
...
function cleanup() {
  ...
  if (gl) {
    ...
    if (resultTex) gl.deleteTexture(resultTex);
    if (resampleFBO) gl.deleteFramebuffer(resampleFBO);
...
// 监听 waterfallData 变化，自动上传并下采样
watch(waterfallData, (arr) => {
  if (arr && arr.length > 0) {
    if (N === 0) {
      N = arr.length;
      // 初始化历史纹理
      ...
      // 初始化缓存
      lastFrameData = new Float32Array(targetN);
    }
    uploadAndResample(arr);
    // 更新缓存
    const targetN = Math.min(N, MAX_POINTS_PER_FRAME);
    gl.bindFramebuffer(gl.FRAMEBUFFER, resampleFBO);
    gl.readPixels(0, 0, targetN, 1, gl.RED, gl.FLOAT, lastFrameData);
