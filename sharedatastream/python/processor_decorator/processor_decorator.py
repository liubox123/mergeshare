...
def ensure_utf8_bytes(data: bytes) -> bytes:
    """
    确保输入数据是有效的 UTF-8 编码字节序列
    如果不是，尝试检测并转换为 UTF-8
    """
    try:
        # 首先尝试解码为字符串，如果成功说明是有效的 UTF-8
        data.decode('utf-8')
        return data
    except UnicodeDecodeError:
        # 如果不是有效的 UTF-8，尝试检测编码
        try:
            ...
            if detected['encoding'] and detected['confidence'] > 0.7:
                ...
                warnings.warn(f"Converted string from {detected['encoding']} to UTF-8", 
                            UserWarning, stacklevel=2)
                return utf8_data
            else:
                # 无法检测编码，使用错误处理策略
                warnings.warn("Unable to detect encoding, using error handling strategy", 
                            UserWarning, stacklevel=2)
                return data.decode('utf-8', errors='replace').encode('utf-8')
        except Exception as e:
            # 转换失败，使用错误处理策略
            warnings.warn(f"Encoding conversion failed: {e}, using error handling strategy", 
                        UserWarning, stacklevel=2)
            return data.decode('utf-8', errors='replace').encode('utf-8')


def validate_and_convert_string(data) -> bytes:
    """
    验证并转换字符串数据为 UTF-8 字节序列
    支持 str 和 bytes 输入
    """
    if isinstance(data, str):
        ...
        # 如果是字节序列，确保是有效的 UTF-8
        return ensure_utf8_bytes(data)
    else:
        # 其他类型，转换为字符串后编码
...
class SharedMemConsumerWrapper:
    def __init__(
        self, in_shm, in_queue_len, in_block_size, batch_size=1, timeout_ms=10
    ):
