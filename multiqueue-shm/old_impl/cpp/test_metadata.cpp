...
TEST(ControlBlockTest, Initialize) {
    ...
    EXPECT_EQ(control.consumers.active_count.load(), 0u);
...
TEST(ControlBlockTest, AtomicOperations) {
    ...
    // 测试消费者注册
    int slot_id = control.consumers.register_consumer("test_consumer", 0);
    EXPECT_GE(slot_id, 0);
    EXPECT_EQ(control.consumers.active_count.load(), 1u);
